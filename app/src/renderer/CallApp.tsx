  ...
  return (
    <main className="call-shell">
      ...
      <section className="call-stage">
        {hasOthers || cameraEnabled ? (
          <section className="remote-grid">
            {/* Local video tile (only when camera is on) */}
            {cameraEnabled && (
              <article className="tile local-tile">
                <h2>You (Jrod)</h2>
            ...
            {remoteTiles.map((tile) => (
              <RemoteVideo key={`${tile.userId}-${tile.version}`} label={tile.displayName} stream={tile.stream} />
            ))}
          </section>
        ) : (
