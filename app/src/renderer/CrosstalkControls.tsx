...
export default function CrosstalkControls() {
  ...
  const { activeCrosstalk, outsideVolume, endCrosstalk, setCrosstalkVolume } = engine;

  if (!activeCrosstalk) return null;

  const isParticipant = activeCrosstalk.participantUserIds.includes(session?.userId ?? "");
  ...
  return (
    <div className="flex items-center gap-2 rounded-md bg-zinc-800 px-2 py-1">
      ...
      <button
        onClick={() => endCrosstalk(activeCrosstalk.id)}
