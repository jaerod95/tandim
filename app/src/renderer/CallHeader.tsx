...
function CrosstalkBanner() {
  ...
  const { activeCrosstalks, myActiveCrosstalk, presence, endCrosstalk } = engine;

  if (activeCrosstalks.length === 0) return null;

  if (myActiveCrosstalk) {
    const otherParticipants = myActiveCrosstalk.participantUserIds
      .filter((id) => id !== engine.localUserId)
    ...
    return (
      <div className="flex items-center gap-2 border-b border-blue-500/20 bg-blue-600/10 px-4 py-1.5">
        ...
        <button
          onClick={() => endCrosstalk(myActiveCrosstalk.id)}
